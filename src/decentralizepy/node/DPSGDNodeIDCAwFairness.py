...
class DPSGDNodeIDCAwFairness(DPSGDNodeIDCAwPS):
    ...
    def init_trainer(self, train_configs):
        ...
        if "loss_class" in train_configs.keys():
            ...
            if self.loss_class == FairLogLossLayerDiff:
                ...
                self.losses = [self.loss_class(lambda_=lambda_, num_blocks=num_blocks) for _ in range(len(self.models))]
            else:
                self.losses = [self.loss_class(lambda_=lambda_) for _ in range(len(self.models))]
        ...
        train_params = utils.remove_keys(
            train_configs,
            ["training_package", "training_class", "loss", "loss_package", "loss_class", "lambda_"],
        )
        ...
        self.fair_metric_dict = {idx: torch.ones(dim).float() * fill for idx in range(len(self.models))}
        self.fair_metric_dict_other = {idx: torch.ones(dim).float() * fill for idx in range(len(self.models))}
    ...
    def get_data_to_send(self) -> Dict:
        ...
        to_send = self.sharing.get_data_to_send(current_idx, fair_metric, len(self.my_neighbors))
        return to_send
    ...
    def compute_log_per_sample_loss_train(self, results_dict: Dict, iteration: int):
        ...
        loss_func.set_fair_metric_other(self.fair_metric_dict_other[self.trainer.current_model_idx])
        if self.trainer.loss_has_model:
            ...
            all_other_models = [model for i, model in enumerate(self.models) if i != self.trainer.current_model_idx]
    ...
    def eval_on_testset(self, results_dict: Dict, iteration):
        ...
        for j, loss in enumerate(self.losses):
            ...
            if self.trainer.loss_has_model:
                ...
                all_other_models = [model for i, model in enumerate(self.models) if i != j]
    ...
    def compute_log_per_sample_metrics_test(self, results_dict: Dict, iteration: int, best_idx: int):
        ...
        if self.trainer.loss_has_model:
            all_other_models = [model for i, model in enumerate(self.models) if i != best_idx]
            other_model = loss_func.compute_other_model(all_other_models)
            loss_func.set_model(model)
            loss_func.set_model_other(other_model)
    ...
    def eval_on_validationset(self, results_dict: Dict, iteration):
        ...
        for loss in self.losses:
            loss.eval()
        va, vl, all_val_loss, bidx = self.dataset.validate(self.models, self.losses)  # losses
