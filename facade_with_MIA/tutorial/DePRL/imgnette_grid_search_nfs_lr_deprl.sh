...
script_path=$nfs_decpy/tutorial/DePRL # Path to the folder where the run_IDCAwPS.sh is located

config_file=config_IMGNETTE_grid_search_deprl.ini

config_file_path=$script_path/configs/$config_file

# m=0 # machine id corresponding consistent with ip.json
ip_machines=$nfs_decpy/tutorial/DePRL/ip_nfs.json # !!!
...
prefix_dir=imgnette_deprl_lenet_lr_$(date '+%Y-%m-%dT%H:%M')
