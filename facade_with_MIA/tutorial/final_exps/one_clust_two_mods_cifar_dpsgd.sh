...
config_file=config_CIFAR_dpsgd.ini
...
# number of models manually modified in the code line 315 of the file src/decentralizepy/node/DPSGDNodeIDCA.py
prefix_dir=one_cluster_dpsgd_mod_$(date '+%Y-%m-%dT%H:%M')
...
$python_bin/crudini --set $run_path/$config_file DATASET number_of_clusters 1
$python_bin/crudini --set $run_path/$config_file DATASET sizes "[[1/32]*32]"
...
log_dir_from_decpy=eval/data/$prefix_dir/$(date '+%Y-%m-%dT%H:%M')
