...
config_file=config_CIFAR.ini
script_path=./tutorial/DisPFL # Path to the folder where the run_IDCAwPS.sh is located
...
iterations=80
test_after=4
eval_file=$decpy_path/testingDisPFL.py # decentralized driver code (run on each machine)
...
log_dir=$run_path/$(date '+%Y-%m-%dT%H:%M')_dispfl/machine$m # in the eval folder
mkdir -p $log_dir
