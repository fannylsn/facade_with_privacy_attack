...
procs_per_machine=8 # =24/4
iterations=801


prefix_dir=imgnette_idca_lenet_shared_$(date '+%Y-%m-%dT%H:%M')
...
# weight_dec=(0.1 0.01 0.001 0.0001)
# weight_dec=(0.1 0.05 0.2)
# for wd in ${weight_dec[@]}

lrs=(0.005)
for lr in ${lrs[@]}
do
    $python_bin/crudini --set $run_path/$config_file OPTIMIZER_PARAMS lr $lr
    # $python_bin/crudini --set $run_path/$config_file OPTIMIZER_PARAMS weight_decay $wd
    ...
    echo lr is $lr
    # echo weight decay is $wd

    log_dir_from_decpy=eval/data/$prefix_dir/share_all_conv_$(date '+%Y-%m-%dT%H:%M')
