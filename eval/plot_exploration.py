...
CONFIGS = ["no_explo", "half_exp_explo", "exp_explo", "half_lin_expl"]
...
def plot(means, stdevs, mins, maxs, title, label, loc, xlabel="communication rounds"):
    ...
    plt.fill_between(x_axis, y_axis - err, y_axis + err, alpha=0.2)
    plt.legend(loc=loc)
...
def plot_results(folder_path, data_machine="machine0", data_node=0):
    ...
    for subdir, results in each_results.items():
        ...
        # remoove all_reduce (comment to deactivate)
        # for x in results:
        #     for key in x.keys():
        #         if isinstance(x[key], dict):
        #             x[key] = {k: v for k, v in x[key].items() if int(k) <= 80}
