...
eval_file=$nfs_decpy/eval/testingIDCAFair.py # decentralized driver code (run on each machine)
script_path=$nfs_decpy/tutorial/IDCA_fair # Path to the folder where the run_IDCAwPS.sh is located

# config_file=configCIFAR_idca_layer.ini
config_file=configIMGNETTE_idca_feat.ini

config_file_path=$script_path/configs/$config_file

# m=0 # machine id corresponding consistent with ip.json
ip_machines=$nfs_decpy/tutorial/ip_files/ip_nfs_0507.json
#ip_machines=$nfs_decpy/tutorial/ip_files/ip.json
...
prefix_dir=testing_idca_imgnette_feat_lrgs_v2_14to2_$(date '+%Y-%m-%dT%H:%M')
...
lrs=(0.001 0.0003) # (0.003 0.001 0.0001) # 0.003 for CIFAR, 0.001 for imagenette
...
do
    ...
    do
        ...
        $python_bin/crudini --set $run_path/$config_file TRAIN_PARAMS lambda_ $lambda
        $python_bin/crudini --set $run_path/$config_file OPTIMIZER_PARAMS lr $lr


        log_dir_from_decpy=eval/data/$prefix_dir/lr_${lr}_lam_${lambda}_$(date '+%Y-%m-%dT%H:%M')
