...
script_path=$nfs_decpy/tutorial/DAC # Path to the folder where the run_IDCAwPS.sh is located

config_file=config_IMGNETTE_grid_search_dac.ini

config_file_path=$script_path/configs/$config_file

# m=0 # machine id corresponding consistent with ip.json
ip_machines=$nfs_decpy/tutorial/DAC/ip_nfs_B.json # !!!
...
prefix_dir=imgnette_dac_lenet_lr_$(date '+%Y-%m-%dT%H:%M')
