...
script_path=./tutorial/MUFFLIATO # Path to the folder where the run_IDCAwPS.sh is located
...
iterations=80
test_after=2
eval_file=$decpy_path/testingIDCAwMUFF.py # decentralized driver code (run on each machine)
...
log_dir=$run_path/$(date '+%Y-%m-%dT%H:%M')_muff/machine$m # in the eval folder
mkdir -p $log_dir
