...
ip_machines=$nfs_decpy/tutorial/combined_runs/ip_nfs.json # !!!
...
prefix_dir=imgnette_round_gs_idfca_$(date '+%Y-%m-%dT%H:%M')

echo "IDCA started at $(date '+%Y-%m-%dT%H:%M')!"
...
do
    ...
    log_dir_from_decpy=eval/data/$prefix_dir/idca_${round}_$(date '+%Y-%m-%dT%H:%M')
...
echo "IFCA started at $(date '+%Y-%m-%dT%H:%M')!"
...
do
    ...
    log_dir_from_decpy=eval/data/$prefix_dir/ifca_${round}_$(date '+%Y-%m-%dT%H:%M')
    log_dir=$machine_decpy/$log_dir_from_decpy/machine$m # in the eval folder
    echo $log_dir
    mkdir -p $log_dir
    start=$(date '+%s')
